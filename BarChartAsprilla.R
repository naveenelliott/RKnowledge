...
df <- read.csv("Player Grades for Each Game - CF's (8).csv", stringsAsFactors = F)
str(df)
new_df = df[,'Date' == '9/18/2022']
# new_df = df[,'Date']
view(new_df)
...
p <- ggplot(df, aes(x = numbers, y = stats)) +
  ...
  labs(title = "Mohamed Salah - Liverpool",
       ...
       tag = "*Adjusted for Premier League") +
...
ggsave(p, file = "MoSalahBarChart.png", width = 17, height = 17, dpi = 300)
